...
function applySession(){
  ...
  document.querySelectorAll('#nav button').forEach(b=>{const perm=PAGE_PERMISSION[b.dataset.page];b.classList.toggle('hidden',!!perm&&!session.permissions.includes(perm));});
  $('page-users')?.classList.toggle('hidden',!session.permissions.includes('users.manage'));
...
function go(page){
  ...
  if(page==='dashboard')loadDashboard();if(page==='sales')loadSales();if(page==='products'){loadProducts().then(()=>{renderProducts();loadPriceHistory()})}if(page==='inventory'){loadProducts().then(()=>{renderProducts();loadPriceHistory()});loadInventory();}if(page==='reports')loadReports();if(page==='customers')loadCustomers();if(page==='cash')loadCash();if(page==='accounting')loadAccounting();if(page==='audit')loadAudit();if(page==='backup')loadBackup();if(page==='sync')loadSync();if(page==='users')loadUsers();
}
